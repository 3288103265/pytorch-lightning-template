...
class ResnetDiscriminator128(nn.Module):
    ...
    def forward(self, x, y=None, bbox=None):
        ...
        return out_im, out_obj, x, obj_feat
...
class ResnetDiscriminator128_app(nn.Module):
    ...
    def forward(self, x, y=None, bbox=None):
        ...
        return out_im, out_obj, out_app, x, obj_feat
...
class CombineDiscriminator128(nn.Module):
    def __init__(self, num_classes=81):
        ...
        self.num_cls = 184  # coco things + stuff
        self.proxy_emb_dim = 1024  # TODO: 1024 maybe too large

        self.proxy_embed = nn.Embedding(self.num_cls, self.proxy_emb_dim)
    ...
    def forward(self, images, bbox, label, mask=None):
        ...
        bbox = bbox[idx]
        label = label[idx]
        d_out_img, d_out_obj, d_out_app, img_feat, obj_feat = self.obD(
            images, label, bbox)
        # filter coco stuff objects which self.num_cls = 91, do nothing when = 184
        idx2 = (label <= self.num_cls).nonzero().view(-1)

        label_proxy = label[idx2]
        obj_feat = obj_feat[idx2]
        cls_feat_proxy = self.proxy_embed(label_proxy)
        return d_out_img, d_out_obj, d_out_app,  obj_feat, cls_feat_proxy, label_proxy
